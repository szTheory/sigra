#!/usr/bin/env bash
# Phase 216 Plan 07 (HARNESS-01): admin-eval harness — thin orchestrator.
#
...
echo "admin-eval-harness: (b3) quality findings monotonic guard"
bash "${ROOT}/scripts/ci/quality-findings-monotonic.sh" --base HEAD

echo "admin-eval-harness: (b4) award ledger verify-then-climb guard"
node "${ROOT}/scripts/ci/award-guard.mjs" --base HEAD

echo "admin-eval-harness: (b5) settled findings lint"
