...
select_run() {
  ...
  for projection in pre post; do
    local file="$before"; [[ "$projection" == post ]] && file="$after"
    jq -e --argjson workflow_id "$workflow_id" '.workflow_runs | all(.[]; .workflow_id == $workflow_id)' "$file" >/dev/null || fail "${projection}_projection_wrong_workflow"
    jq -e '.workflow_runs | all(.[]; .event == "workflow_dispatch")' "$file" >/dev/null || fail "${projection}_projection_wrong_event"
    jq -e --arg sha "$head_sha" '.workflow_runs | all(.[]; .head_sha == $sha)' "$file" >/dev/null || fail "${projection}_projection_wrong_sha"
  done
  ...
  candidates="$(jq -cn --slurpfile pre "$before" --slurpfile post "$after" --argjson workflow_id "$workflow_id" --arg sha "$head_sha" --arg boundary "$not_before" '
    ...
    | [ $post[0].workflow_runs[]
        ...
        | select(.workflow_id == $workflow_id and .event == "workflow_dispatch" and .head_sha == $sha)
        ...
        | {id, html_url, workflow_id, event, head_sha, created_at}
      ]
  ')"
...
self_test() {
  ...
  jq -e '.id == 42 and .html_url == "https://example.test/42"' "$out" >/dev/null
  for mode in zero multiple stale wrong_workflow wrong_event wrong_sha; do
    case "$mode" in
      zero) printf '%s\n' '{"total_count":0,"workflow_runs":[]}' >"$after" ;;
      multiple) printf '%s\n' '{"total_count":2,"workflow_runs":[{"id":42,"html_url":"https://example.test/42","workflow_id":7,"event":"workflow_dispatch","head_sha":"abc","created_at":"2026-08-03T00:00:00Z"},{"id":43,"html_url":"https://example.test/43","workflow_id":7,"event":"workflow_dispatch","head_sha":"abc","created_at":"2026-08-03T00:00:01Z"}]}' >"$after" ;;
      stale) printf '%s\n' '{"total_count":1,"workflow_runs":[{"id":42,"html_url":"https://example.test/42","workflow_id":7,"event":"workflow_dispatch","head_sha":"abc","created_at":"2026-08-02T23:59:59Z"}]}' >"$after" ;;
      wrong_workflow) printf '%s\n' '{"total_count":1,"workflow_runs":[{"id":42,"html_url":"https://example.test/42","workflow_id":8,"event":"workflow_dispatch","head_sha":"abc","created_at":"2026-08-03T00:00:00Z"}]}' >"$after" ;;
      wrong_event) printf '%s\n' '{"total_count":1,"workflow_runs":[{"id":42,"html_url":"https://example.test/42","workflow_id":7,"event":"push","head_sha":"abc","created_at":"2026-08-03T00:00:00Z"}]}' >"$after" ;;
      wrong_sha) printf '%s\n' '{"total_count":1,"workflow_runs":[{"id":42,"html_url":"https://example.test/42","workflow_id":7,"event":"workflow_dispatch","head_sha":"nope","created_at":"2026-08-03T00:00:00Z"}]}' >"$after" ;;
    esac
    ...
    case "$mode" in
      zero|stale) expected='candidate_count_0' ;;
      multiple) expected='candidate_count_2' ;;
      wrong_workflow) expected='post_projection_wrong_workflow' ;;
      wrong_event) expected='post_projection_wrong_event' ;;
      wrong_sha) expected='post_projection_wrong_sha' ;;
    esac
