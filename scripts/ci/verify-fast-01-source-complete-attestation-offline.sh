...
for command in gh jq mktemp realpath shasum; do command -v "$command" >/dev/null || fail "missing_required_command:$command"; done
...
GH_BIN="$(realpath "$(command -v gh)")"; JQ_BIN="$(realpath "$(command -v jq)")"; MKTEMP_BIN="$(realpath "$(command -v mktemp)")"
...
' "$RECEIPT" >/dev/null || fail "source_first_semantic_validation_failed"
