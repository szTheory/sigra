...
if [[ $event == workflow_dispatch ]]; then [[ $(jq -r .path <<<"$run") == .github/workflows/ci.yml ]] || fail dispatch_workflow_invalid; elif [[ $event != push ]]; then fail run_event_invalid; fi
...
jq -e --argjson id "$id" 'all(.[];.run_id==$id and (.id|type)=="number" and .status=="completed" and (.conclusion|type)=="string" and (.steps|type)=="array") and ([.[].id]|unique|length)==length' "$t/jobs" >/dev/null || fail jobs_invalid
