...
- PostgreSQL running
- `{:sigra, "~> 1.4.0"}` in `mix.exs` and `mix deps.get` already run. If you are reading `main` before Hex shows `1.0.0`, use the latest published Sigra package or a source checkout until the release PR lands.
