defmodule Sigra.Config do
  ...
  @schema [
    ...
    jwt: [
      ...
      keys: [
        ...
        audience: [
          type: {:custom, Sigra.Config, :validate_jwt_audiences, []},
          default: ["sigra"],
          doc: "Non-empty unique accepted JWT audience values."
        ],
        typ: [
          type: {:custom, Sigra.Config, :validate_jwt_typ, []},
          default: "JWT",
          doc: "Required protected JWT type header."
        ],
        access_ttl: [
          type: :pos_integer,
          default: 900,
  ...
  @doc false
  @spec validate_jwt_audiences(term()) :: {:ok, [String.t()]} | {:error, String.t()}
  def validate_jwt_audiences(audiences) when is_list(audiences) and audiences != [] do
    if Enum.all?(audiences, &(is_binary(&1) and byte_size(&1) > 0)) and
         length(audiences) == MapSet.size(MapSet.new(audiences)) do
      {:ok, audiences}
    else
      {:error, "must contain unique non-empty strings"}
    end
  end

  def validate_jwt_audiences(_), do: {:error, "must be a non-empty list of unique strings"}
