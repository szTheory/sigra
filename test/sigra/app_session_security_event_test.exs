defmodule Sigra.AppSessionSecurityEventTest do
  ...
  setup_all do
    Sigra.Test.PostgresCase.checkout_repo!(fn repo ->
      Ecto.Adapters.SQL.query!(repo, "ALTER TABLE sigra_app_session_users ADD COLUMN IF NOT EXISTS hashed_password text", [])
  ...
  setup %{repo: repo} do
    {:ok, user} = repo.insert(%User{email: "security-event-#{System.unique_integer([:positive])}@example.com", hashed_password: "old"})
    %{repo: repo, user: user}
  end

  test "password reset revokes app credentials in its transaction and leaves no secret in audit metadata", %{repo: repo, user: user} do
    config = config(repo)
    assert {:ok, %{access_token: access, refresh_token: refresh, family_id: family_id}} = AppSession.issue(config, user, "reset-client")
    ...
    assert repo.aggregate(Ecto.Query.from(t in UserToken, where: t.user_id == ^user.id), :count, :id) == 0

    [event] = repo.all(Ecto.Query.from(e in AuditEvent, where: e.action == "auth.password_reset_complete"))
    refute inspect(event.metadata) =~ access
    refute inspect(event.metadata) =~ refresh
  end

  test "password-reset audit rejection rolls back password, reset token, and app-family revocation", %{repo: repo, user: user} do
    config = config(repo)
    assert {:ok, %{family_id: family_id}} = AppSession.issue(config, user, "rollback-client")
    {encoded, reset_token} = reset_token(user)
    {:ok, token_record} = repo.insert(reset_token)

    Ecto.Adapters.SQL.query!(repo, "ALTER TABLE audit_events ADD CONSTRAINT security_event_reset_audit CHECK (action <> 'auth.password_reset_complete')", [])
    ...
    after
      Ecto.Adapters.SQL.query!(repo, "ALTER TABLE audit_events DROP CONSTRAINT IF EXISTS security_event_reset_audit", [])
    end
  end

  test "sign-out-all revokes only the target user's app credentials even when retaining a browser token", %{repo: repo, user: user} do
    config = config(repo)
    {:ok, other} = repo.insert(%User{email: "other-#{System.unique_integer([:positive])}@example.com", hashed_password: "old"})
    ...
    assert {0, nil} =
             Auth.delete_all_sessions(%{config | session: [store: Sigra.MockSessionStore, session_schema: User]}, user.id,
  ...
  defp config(repo), do: Sigra.Config.new!(repo: repo, user_schema: User, app_session: [family_schema: Family, token_schema: AppToken])

  defp reset_opts(config), do: [secret_key_base: "security-event-secret", user_token_schema: UserToken, user_schema: User, changeset_fn: fn user, attrs -> Ecto.Changeset.change(user, hashed_password: attrs["password"]) end, app_session_config: config, audit_schema: AuditEvent]
  ...
  defp reset_token(user) do
    ...
    {Base.url_encode64(signed, padding: false), %UserToken{token: Token.hash_token(raw), context: "reset_password", sent_to: user.email, user_id: user.id}}
  end
end
