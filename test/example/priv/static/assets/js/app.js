...
(function() {
  ...
  function safeLoginStatus(error) {
    ...
    if (error && error.code === ERROR_PASSKEY_UNSUPPORTED) {
      return {
        status: "unsupported",
  ...
  function attachPasskeyLogin(options) {
    ...
    var emailInput = form.querySelector("input[name='user[email]']");
    ...
    function authenticateExplicit(event) {
      ...
      return fetchJsonOptions(optionsUrl, { user: { email: email } }).then(function(optionsJSON) {
        ...
        updateLoginStatus(form, { name: "Success", code: null, message: "" });
