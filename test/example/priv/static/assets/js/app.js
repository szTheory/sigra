...
(function() {
  ...
  function serializeCredential(credential) {
    ...
    return {
      ...
      response: {
        ...
        attestationObject: response.attestationObject ? base64UrlEncode(new Uint8Array(response.attestationObject)) : null,
        authenticatorData: response.authenticatorData ? base64UrlEncode(new Uint8Array(response.authenticatorData)) : null,
        signature: response.signature ? base64UrlEncode(new Uint8Array(response.signature)) : null,
  ...
  function safeLoginStatus(error) {
    ...
    if (error && (error.code === ERROR_PASSKEY_UNSUPPORTED || error.name === "NotSupportedError")) {
      return {
        status: "unsupported",
        message: "Passkeys aren't available in this browser."
      };
    }
    if (error && (error.code === CEREMONY_ABORTED || error.name === "AbortError" || error.name === "NotAllowedError")) {
      return {
        status: "canceled",
        message: "Passkey sign-in was canceled."
      };
    }
    if (error && (error.name === "TimeoutError" || error.code === "ERROR_CEREMONY_TIMEOUT")) {
      return {
        status: "timeout",
  ...
  function updateLoginStatus(form, errorOrStatus) {
    ...
    var status = typeof errorOrStatus === "string" ? safeLoginStatus(errorOrStatus) : safeLoginStatus(errorOrStatus);
  ...
  function findEmailInput(form, options) {
    ...
    return (form.dataset.emailInput ? document.querySelector(form.dataset.emailInput) : null) || form.querySelector("input[name='user[email]']:not([data-passkey-email-shadow])");
  }
  ...
  function startRegistration(params) {
    return navigator.credentials.create({
      publicKey: {
        ...
        excludeCredentials: (params.optionsJSON.excludeCredentials || []).map(function(credential) {
          return {
  ...
  function startAuthentication(params) {
    var request = {
      publicKey: {
        ...
        allowCredentials: (params.optionsJSON.allowCredentials || []).map(function(credential) {
          return {
            id: toUint8Array(credential.id),
            type: credential.type,
            transports: credential.transports
    ...
    var mediation = params.useBrowserAutofill === true ? conditionalMediationAvailable().then(function(available) {
      if (!available) {
        throw new WebAuthnError("unsupported", ERROR_PASSKEY_UNSUPPORTED);
      }
      request.mediation = "conditional";
    }) : Promise.resolve();
    return mediation.then(function() {
    ...
    return {
      mounted: function() {
        ...
        this.handleEvent(config.startEvent, function(payload) {
          ...
          var optionsPromise = payload.options ? Promise.resolve(payload.options) : payload.optionsUrl ? fetchJsonOptions(payload.optionsUrl, payload.optionsBody || {}) : Promise.resolve(null);
          optionsPromise.then(function(optionsJSON) {
            return config.startCeremony(payload, optionsJSON, abortController.signal);
          }).then(function(response) {
            if (!_this.isLatestPasskeyOperation(operationId) || abortController.signal.aborted) {
              return;
            }
            _this.pushEvent(config.successEvent, { response: toPlainObject(response) });
  ...
  function attachPasskeyLogin(options) {
    ...
    var button = options.button || document.querySelector("#passkey_login_button");
    if (!form || !button) {
    ...
    var optionsUrl = options.optionsUrl || form.dataset.optionsUrl || form.dataset.optionsPath || "/users/log_in/passkey/options";
    var completeUrl = options.completeUrl || form.action || "/users/log_in/passkey";
    function authenticateExplicit(event) {
      ...
      return fetchJsonOptions(optionsUrl, { user: { email: email } }).then(function(optionsJSON) {
        return startAuthentication({
          optionsJSON: optionsJSON,
    ...
    var ready = options.enableConditionalUI === true ? conditionalMediationAvailable().then(function(available) {
      if (!available) {
        throw new WebAuthnError("unsupported", ERROR_PASSKEY_UNSUPPORTED);
      }
      return fetchJsonOptions(optionsUrl, { conditional: "true" }).then(function(optionsJSON) {
        return startAuthentication({
          optionsJSON: optionsJSON,
...
(() => {
  ...
  function readBrandState() {
    ...
    try {
      ...
      const defaultId = host.dataset.demoBrandDefault || (presets[0] && presets[0].id);
  ...
  function storedTheme() {
    ...
    try {
      return normalizeTheme(window.localStorage && window.localStorage.getItem(THEME_STORAGE_KEY));
    } catch (_error) {
      return null;
    }
  }
  ...
  function profileForTheme(preset, theme) {
    ...
    return profiles[variant] || preset.profile || profiles.light || profiles.dark || {};
  }
  ...
  function setText(root, key, value) {
    root.querySelectorAll(`[data-demo-brand-text="${key}"]`).forEach(element => {
  ...
  function applyVariantStyles(root, preset, theme, tokens) {
    ...
    for (const variant of ["light", "dark"]) {
      ...
      for (const [property, key] of tokens) {
        if (profile[key]) {
          root.style.setProperty(variantProperty(property, variant), profile[key]);
        }
      }
    }
  ...
  function applyBrandAssets(profile) {
    ...
    const logoAlt = profile.logo_alt || (productName ? `${productName} logo` : "Brand logo");
    ...
    document.querySelectorAll("[data-demo-brand-fallback-mark]").forEach(element => {
      element.hidden = Boolean(logoUrl);
    });
    document.querySelectorAll("[data-demo-brand-initial]").forEach(element => {
      element.textContent = productName.trim().slice(0, 1).toUpperCase() || "?";
      element.hidden = Boolean(logoUrl);
    });
  }
  ...
  function applyPreset(state, id, options = {}) {
    ...
    document.querySelectorAll("[data-demo-brand-description]").forEach(element => {
      element.textContent = preset.description || "";
    });
    document.querySelectorAll("[data-demo-brand-subject]").forEach(element => {
      element.textContent = preset.email_subject || "";
    });
    applyBrandAssets(profile);
    document.querySelectorAll("[data-demo-auth-preview], [data-demo-email-preview]").forEach(element => {
      applyVariantStyles(element, preset, theme, sigraStyleTokens);
    });
    document.querySelectorAll("[data-demo-brand-surface]").forEach(element => {
  ...
  function initDemoBranding() {
    ...
    document.addEventListener("change", event => {
      const select = event.target && event.target.closest && event.target.closest("[data-demo-brand-select]");
      if (select) {
      ...
      const themeInput = event.target && event.target.closest && event.target.closest("[data-demo-brand-theme]");
      if (themeInput) {
