defmodule SigraInstallGoldenTmpWeb.EnterpriseSSOController do
  @moduledoc """
  ...
  def create(conn, %{"org" => org_slug} = params) do
    with {:ok, routing} <- Organizations.get_routable_enterprise_connection(org_slug),
         {:ok, authorize_url, session_params} <-
           oauth_module().authorize_url(enterprise_sigra_config(routing.connection), :oidc,
             enterprise: %{
  ...
  defp oauth_module do
    Application.get_env(:sigra_install_golden_tmp, :enterprise_oauth_module, Sigra.OAuth)
  end

  defp enterprise_sigra_config(connection) do
    base_config = Auth.sigra_config()

    oauth_config =
      base_config
      |> Map.get(:oauth, [])
      |> Keyword.merge(
        providers: [oidc: enterprise_provider_config(connection)],
        trust_provider_email: true,
        session_type: :remember_me,
        link_confirmation: :required
      )

    %{base_config | oauth: oauth_config}
  end

  defp enterprise_provider_config(connection) do
    settings = connection.oidc_settings
    ...
    [
      ...
      client_secret: settings.encrypted_client_secret,
      client_authentication_method: settings.client_authentication_method,
