defmodule SigraInstallGoldenTmpWeb.ResetPasswordController do
  @moduledoc """
  ...
  def update(conn, %{"token" => token, "user" => password_params}) do
    case SigraInstallGoldenTmp.Accounts.get_user_by_reset_password_token(token) do
      ...
      user ->
        ...
        case SigraInstallGoldenTmp.Accounts.reset_user_password(token, password_params) do
          {:ok, _user} ->
            conn
            |> put_flash(:info, dgettext("sigra", "Your password has been reset."))
            |> SigraInstallGoldenTmpWeb.UserAuth.log_in_user(user)
