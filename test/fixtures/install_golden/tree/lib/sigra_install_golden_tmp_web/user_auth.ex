defmodule SigraInstallGoldenTmpWeb.UserAuth do
  @moduledoc """
  ...
  alias SigraInstallGoldenTmp.Accounts.Scope

  # Make the remember me cookie valid for 60 days.
  # If you want bump or reduce this value, also change
  # the token expiry itself in UserToken.
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_sigra_install_golden_tmp_user_remember_me"
  ...
  defp remember_me_options do
    config = SigraInstallGoldenTmp.Accounts.sigra_config()
  ...
  def fetch_current_scope(conn, _opts) do
    ...
    {user, session} =
      case user_token && SigraInstallGoldenTmp.Accounts.get_user_and_session_by_token(user_token) do
        {u, s} -> {u, s}
        _ -> {nil, nil}
      end
  ...
  the current_scope:
      ...
      defmodule SigraInstallGoldenTmpWeb.PageLive do
        use SigraInstallGoldenTmpWeb, :live_view

        on_mount {SigraInstallGoldenTmpWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{SigraInstallGoldenTmpWeb.UserAuth, :ensure_authenticated}] do
