...
findings:
  - element: "<.scope_ribbon> rendered after <header> but BEFORE <form> (audit_index_live.ex:56)"
    ...
    resolution: "FIXED (Phase 209-05 Task 1) — <.scope_ribbon> moved above <header class='sg-page-header'> in audit_index_live.ex, matching the canonical before-header position on user_show_live, user_sessions_live, and audit_user_live siblings. UI-SPEC Interaction Contract enforced: 'Scope ribbon position — Above the page <header> on every list/leaf page; NOT below.'"
  - element: "<.applied_chip> cluster rendered OUTSIDE the <form> (audit_index_live.ex:142-149)"
    ...
    disposition_action: tighten
    resolution: "WAIVER (Phase 209-05 Task 1) — Post-form chip position is the DEFINED Audit Explorer Archetype pattern per admin-design-contract.md (Audit Explorer Archetype, position [3]: 'Navigation-only <a> tags, post-form'). The applied_chip component contract itself (admin-design-contract.md:applied_chip) cites 'audit_user_live.ex applied-chip cluster (post-form, contiguous with filter panel — see Audit Explorer Archetype for elevated composition)'. The List Archetype uses chips-inside-form (D-01 — chips participate in GET submission); the Audit Explorer Archetype uses chips post-form (chips are navigation-only <a> tags; no named inputs). These are intentionally different archetypes with different composition rules. No DOM change to chips — this asymmetry is the defined per-archetype pattern, not a coherence gap."
---
