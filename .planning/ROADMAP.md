...
**Goal:** Verify by human execution that the OAuth generator (`mix sigra.gen.oauth`) and the live Google OAuth flow (register, login, link, unlink, email-match confirmation) work end-to-end with real Google developer credentials on a fresh Phoenix 1.8 host. Establishes that the OAuth surface that machine tests already cover at the contract level also works through a real consent screen and a real redirect.
...
1. A reviewer reading `.planning/uat-evidence/v1.20/oauth-gen/` sees a terminal transcript proving `mix sigra.gen.oauth` on a freshly-generated Phoenix 1.8 app emitted ≥12 files, wired routes/config/vault child supervisor, and produced a host that compiles `--warnings-as-errors` clean and passes `mix test`.
2. A reviewer opening `.planning/uat-evidence/v1.20/oauth-google/` finds a screen recording or staged screenshot sequence for: provider button click → Google consent → callback redirect → user record created → session established → logout → re-login via the same provider — with timestamps and a pass/fail verdict.
3. A reviewer can confirm provider linking and last-method unlink prevention by inspecting screenshots of all four UI states (linked / disabled-unlink-tooltip / password-set / unlinked-after-password) plus a screenshot of the `user_identities` row check.
4. A reviewer can confirm the email-match confirmation flash + redirect by inspecting screenshots of the flash message, the post-login destination URL, and a `user_identities` row created for the existing user, all filed under `.planning/uat-evidence/v1.20/oauth-google/`.
