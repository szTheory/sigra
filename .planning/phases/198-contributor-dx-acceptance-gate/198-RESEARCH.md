...
> Note: there is a *second*, real `continue-on-error: true` YAML key at ci.yml:1612 on the OQ3 cross-lane compare step inside `admin_design_recapture` (a nightly-only measurement step) — that one is intentional and should stay. Don't touch it.
>
> GREP-COUNT TRAP (read before writing any count-based assertion): a bare `grep -c 'continue-on-error: true' ci.yml` returns **3**, not 2 — ci.yml:1047 (the design_gallery soft-gate, the only one to remove), ci.yml:1612 (the real OQ3 key, keep), and ci.yml:1608, which is a *comment* line ("# continue-on-error: true so a font-driven compare failure...") in the OQ3 doc-comment that the bare substring also matches. After removing only 1047, the bare count is still **2** (1608 comment + 1612 key). Do NOT delete the 1608 comment or the 1612 key to chase a count. Count real keys with the comment-immune anchor `grep -cE '^[[:space:]]*continue-on-error: true' ci.yml` — **2 before the edit, 1 after**.

## Landmines (read before planning)

| # | Landmine | Detail | Mitigation |
|---|----------|--------|------------|
| 1 | **Design-gallery still soft-gated** | ci.yml:1047 `continue-on-error: true` (TEMP); aggregator omits design_gallery. NB: a bare `grep -c 'continue-on-error: true'` = **3** (1047 soft-gate + 1612 real OQ3 key + 1608 OQ3 *comment* line) and = **2** after removing only 1047 — count real keys with `grep -cE '^[[:space:]]*continue-on-error: true'` (2 → 1). | Recapture merged (#60) → unblocked; remove **only** the 1047 continue-on-error + restore aggregator loop entry. Do NOT delete the 1608 comment or the 1612 key. See above. |
