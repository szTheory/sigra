...
| 21-10 | 8 | PK-UX-01..PK-UX-12 | T-21-10-01..03 | Gap closure runtime fixes ensure normal passkey options issuance never signs nil challenge bytes and persisted `"mfa_pending"` session rows hydrate as `:mfa_pending` | library runtime regression | `mix test test/sigra/plug/passkey_challenge_test.exs test/sigra/session_stores/ecto_test.exs` | ⬜ pending |
| 21-11 | 9 | PK-UX-01..PK-UX-12 | T-21-11-01..03 | Gap closure route and browser proof exercises real options endpoints, route-backed MFA passkey success, and Playwright without options interception against a self-started example server | example route + self-contained Playwright | `cd test/example && PGUSER=postgres PGPASSWORD=postgres PGHOST=localhost mix test test/example_web/controllers/passkey_session_controller_test.exs --max-failures 1 && cd ../.. && bash -lc 'set -euo pipefail; cd test/example; MIX_ENV=dev PGUSER=postgres PGPASSWORD=postgres PGHOST=localhost mix ecto.setup; MIX_ENV=dev PGUSER=postgres PGPASSWORD=postgres PGHOST=localhost mix phx.server > /tmp/sigra-example-playwright.log 2>&1 & server_pid=$!; cleanup(){ kill "$server_pid" 2>/dev/null || true; wait "$server_pid" 2>/dev/null || true; }; trap cleanup EXIT; for i in {1..60}; do if curl -fsS http://localhost:4000/users/log_in >/dev/null; then break; fi; if ! kill -0 "$server_pid" 2>/dev/null; then cat /tmp/sigra-example-playwright.log; exit 1; fi; sleep 1; done; curl -fsS http://localhost:4000/users/log_in >/dev/null; cd priv/playwright; SIGRA_EXAMPLE_URL=http://localhost:4000 npx playwright test tests/passkey-login.spec.ts'` | ⬜ pending |
...
All implementation tasks in the eleven PLAN.md files include `<verify><automated>...`.
...
- Plan 21-10 adds focused regression coverage for server-generated passkey challenge bytes and `"mfa_pending"` Ecto session hydration before changing runtime code.
- Plan 21-11 extends route-backed example tests and updates Playwright to observe the real `/users/log_in/passkey/options` response against a server started by the verification command itself.
