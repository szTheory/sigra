...
| 21-04 | 2 | PK-UX-06, PK-UX-07, PK-UX-08, PK-UX-10, PK-UX-11, PK-UX-12 | T-21-04-01..09 | Passkey-primary login stays identifier-first, has `attachPasskeyLogin()` DOMContentLoaded binding for the controller page, signup enrollment is config-gated, unconfirmed users cannot use passkey-primary, magic-link recovery remains mandatory, and conditional UI is progressive | generator + JS | `PGUSER=postgres PGPASSWORD=postgres PGHOST=localhost mix test test/sigra/install/generator_passkey_primary_login_test.exs test/sigra/install/generator_wiring_test.exs test/sigra/install/features/passkeys_js_test.exs --max-failures 1` | ⬜ pending |
