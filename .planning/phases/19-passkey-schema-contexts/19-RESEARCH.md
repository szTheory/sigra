...
> All questions resolved during 2026-04-15 planning + revision pass. Each item below carries an inline **RESOLVED** marker with citation.

1. **COSE key serialization format.** **RESOLVED (2026-04-15, Plan 19-01 D-17):** Locked on `:erlang.term_to_binary/1` + `:erlang.binary_to_term(bin, [:safe])`. Zero new deps, round-trips Elixir maps with integer keys exactly, encrypted column stores opaque bytes anyway, matches how `Sigra.MFA` stores TOTP raw secret binaries. The `[:safe]` decode flag prevents atom-creation DoS. Documented in Plan 19-01 `<interfaces>` block as **D-17**. Empirically validated by the D-16 `wax_` round-trip smoke test in Plan 19-01 Task 1 (`test/sigra/passkeys/wax_roundtrip_test.exs` Test 4: `CoseKey.deserialize(CoseKey.serialize(cose)) == cose` on a real `wax_`-returned COSE key).

2. **Where does the audit payload land in `log_multi_safe/3`?** **RESOLVED (2026-04-15, revision pass):** The field name is `:metadata`. Verified by grep of `lib/sigra/audit.ex:448` (`build_attrs/4`: `metadata: Keyword.get(opts, :metadata, %{})`) and the generated schema template `priv/templates/sigra.install/core/audit_event.ex:36` (`field :metadata, :map, default: %{}`). Plan 19-03 D-10 audit payload is emitted via `log_multi_safe("passkey.sign_count_regression", passkey_audit_opts(config) ++ [metadata: %{credential_id: ..., previous_count: ..., presented_count: ..., policy_applied: ..., delta: ..., rp_id: ...}])`. Plan 19-02 Task 2 `passkey.register.success` audit call uses the same `metadata:` key.

3. **Does Plan 19-04 vault promotion need a data migration for existing MFA rows under the stub?** **RESOLVED (2026-04-15, revision pass):** Plan 19-04 Task 3 now includes an explicit integration test (`test/sigra/install/vault_promotion_test.exs` — "pre-existing stub-written MFA rows survive promotion" case) that: (a) seeds `UserMFACredential` rows via the stub encryption path so `encrypted_secret` holds plaintext, (b) runs `promote_vault/1`, (c) queries the raw table via `Ecto.Adapters.SQL.query!/3`, (d) asserts the raw `encrypted_secret` column no longer contains the plaintext secret substring. The Phase 19 threat register updates **T-19-04-07** from "accept" to "mitigate — integration test added". Reference: Plan 19-04 Task 3 `<behavior>` Test 7.

4. **`credential_id` column size (MySQL/SQLite).** **RESOLVED (2026-04-15, revision pass):** Deferred to the secondary-adapter phase. Per `CLAUDE.md` the blessed path is Postgres, and Phase 19 targets Postgres only — the migration template uses `:binary` / `bytea` with no size cap. MySQL/SQLite adapters pick their own limit (spec-safe answer is `VARBINARY(1023)` per W3C WebAuthn §5.8.3) when they land; this is out of Phase 19 scope per CLAUDE.md "PostgreSQL as primary".

5. **`Sigra.Passkeys.known_transport?/1` visibility.** **RESOLVED (2026-04-15, Plan 19-02 Task 2 — revision pass):** Public. Host apps need it for their own telemetry / dashboards per CONTEXT.md D-02. Added explicitly to Plan 19-02 Task 2 action (`def known_transport?(t) when t in ~w(usb nfc ble internal hybrid)a, do: true; def known_transport?(_), do: false`) with a one-case unit test in acceptance criteria.
