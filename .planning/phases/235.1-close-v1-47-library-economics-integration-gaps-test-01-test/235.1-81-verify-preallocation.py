#!/usr/bin/env python3
"""Derive and verify Plan 81's zero-effect sequential-authority preallocation."""
...
SCHEMA = "sigra.phase235.1-plan81-preallocation-validation/v1"
ROOT_TEMPLATE = "/private/tmp/sigra-p2351-plan81-sequential-authority.XXXXXXXX"
...
PREDECESSORS = {
    ...
    "235.1-70-PLAN.md": "0cf4db4405d228a6ae36c388e608e247a7612e60416691cdfe5ce6956142c9f5",
    ...
    "235.1-80-PLAN.md": "cc3731c6e12c787f6bedcbe9a0973280b597f61f599f96a658ebcfee0d1fc61d",
    "235.1-80-verify-preallocation.py": "ede4d9124a2e3c69ef5dcdef8be19a74f01135f839ae91a2fb458d6d924abd80",
    "235.1-80-PREALLOCATION-VALIDATION.json": "9e3f558fe652f11f55b6e11c33659d7c10e88f54ec89051a93c5c06e6a00fd6f",
    "235.1-80-PREALLOCATION-FAILURE.json": "61ab41b211383bc7913753b356eeacb272208bbaa67c4e3d9c389f6ddc054d44",
    "235.1-80-SUMMARY.md": "8e3b2bd461cb9d1e5267d311f403de23e6c503bbaf5f618f77995603957af4b0",
}
...
REMOTE_IDS = [
    "plan81-candidate-push",
    "plan81-capture-pull-request",
    "plan81-capture-workflow-dispatch",
    "plan81-evidence-child-push",
    "plan81-validation-pull-request",
    "plan81-validation-workflow-dispatch",
]
DIAGNOSTIC = {
    "action_id": "plan81-classified-transient-diagnostic-retry",
    "run_attempt_identity_slot": "plan81-classified-transient-diagnostic-run-attempt",
    "monitor_identity_slot": "plan81-classified-transient-diagnostic-monitor",
...
def validate_plan76_pair() -> dict[str, Any]:
    ...
    for row, receipt_path in zip(partitions, partition_paths):
        ...
        require(row["manifest_sha256"] == MANIFESTS[str(pid)], f"Plan 76 partition {pid} manifest drifted")
...
def validate_plan80(inputs: dict[str, bytes]) -> dict[str, Any]:
    summary = text(inputs["235.1-80-SUMMARY.md"], "Plan 80 summary")
    verifier = text(inputs["235.1-80-verify-preallocation.py"], "Plan 80 verifier")
    candidate = json.loads(
        text(inputs["235.1-80-PREALLOCATION-VALIDATION.json"], "Plan 80 candidate"),
        object_pairs_hook=pairs_object,
    )
    failure = json.loads(
        text(inputs["235.1-80-PREALLOCATION-FAILURE.json"], "Plan 80 failure"),
        object_pairs_hook=pairs_object,
    )
    require(front_status(summary, "Plan 80 summary") == "halted", "Plan 80 summary is not halted")
    require(candidate.get("schema") == "sigra.phase235.1-plan80-preallocation-validation/v1", "Plan 80 candidate schema drifted")
    require(candidate.get("status") == "PASS", "Plan 80 excluded candidate shape drifted")
    require(set(candidate.get("partition_authority", {}).get("manifest_sha256", {})) == {"1", "2"}, "Plan 80 serialized manifest keys drifted")
    require(re.search(r"^MANIFESTS = \{\n\s+1:", verifier, re.M) is not None, "Plan 80 integer-key failure source drifted")
    require(failure.get("schema") == "sigra.phase235.1-plan80-preallocation-failure/v1", "Plan 80 failure schema drifted")
    require(failure.get("status") == "FAIL", "Plan 80 failure status drifted")
    require(failure.get("stage") == "task1-immediate-independent-reverification-before-allocation", "Plan 80 failure stage drifted")
    detail = failure.get("failure", {})
    require(detail.get("type") == "deterministic-verifier-roundtrip-mismatch", "Plan 80 failure type drifted")
    require(detail.get("cause") == "partition_authority.manifest_sha256 is derived with integer mapping keys; JSON serialization/reload produces string keys, so exact equality is false", "Plan 80 failure cause drifted")
    require(detail.get("fix_attempts_consumed") == detail.get("fix_attempt_limit") == 3, "Plan 80 fix-attempt ledger drifted")
    require(detail.get("retry_permitted") is False, "Plan 80 retry authority drifted")
    require(failure.get("effects") == {
        "filesystem_root_allocation": 0,
        "local_terminal_launcher": 0,
        "timing_children": 0,
        "exact_six": 0,
        "git_remote_actions": 0,
        "github_api": 0,
        "watchers": 0,
        "capture": 0,
        "validation": 0,
    }, "Plan 80 zero-effect ledger drifted")
    require(failure.get("remaining") == {
        "filesystem_root_allocation": 1,
        "local_terminal_launcher": 1,
        "timing_children": 4,
        "exact_six": 1,
        ...
        "validation_pull_request": 1,
        "validation_workflow_dispatch": 1,
        "diagnostic_retry_triplet": 1,
    }, "Plan 80 remaining ledger drifted")
    require(failure.get("plan76", {}).get("execution_mode_observed") == "sequential", "Plan 80 Plan 76 observation drifted")
    require(failure.get("plan76", {}).get("admitted_for_plan80") is False, "Plan 80 candidate was incorrectly admitted")
    require(not glob.glob("/private/tmp/sigra-p2351-plan80-sequential-authority.*"), "Plan 80 root exists")
    return {
        "status": "halted",
        "candidate_status": "excluded_pass_shaped_negative_evidence",
        "failure": failure,
        "root_count": 0,
...
def derive_schedule() -> list[dict[str, Any]]:
    ...
    for repeat in (1, 2):
        for partition in (1, 2):
            action = f"plan81-repeat-{repeat}-partition-{partition}"
            rows.append({
                "repeat": repeat,
...
def scheduler_authority(root: Path, inputs: dict[str, bytes]) -> dict[str, Any]:
    ...
    require(raw["phase_plan_index"]["incomplete"] == ["235.1-70", "235.1-81"] and raw["phase_plan_index"]["runnable"] == ["235.1-70", "235.1-81"], "phase-plan-index active set drifted")
    require(raw["init_execute_phase"]["incomplete_plans"] == ["235.1-70-PLAN.md", "235.1-81-PLAN.md"] and raw["init_execute_phase"]["runnable_plans"] == ["235.1-70-PLAN.md", "235.1-81-PLAN.md"], "init.execute-phase active set drifted")
    p81 = text(load_bytes(root / PHASE / "235.1-81-PLAN.md"), "Plan 81")
    p70 = text(inputs["235.1-70-PLAN.md"], "Plan 70")
    require(re.search(r"^wave:\s*1$", p81, re.M) and re.search(r"^depends_on:\s*\[\]$", p81, re.M), "Plan 81 dependency metadata drifted")
    require(re.search(r"^wave:\s*2$", p70, re.M) and re.search(r"^depends_on:\s*\[235\.1-81\]$", p70, re.M), "Plan 70 dependency metadata drifted")
    require("Plan 81" in p70 and "five" in p70 and "PASS" in p70, "Plan 70 success authority drifted")
    return {"raw": raw, "derived": {"execution_chain": ["235.1-81", "235.1-70"], "superseded_absent": ["235.1-04", "235.1-10", "235.1-12", "235.1-15"]}}
...
def derive_expected(root: Path) -> dict[str, Any]:
    ...
    roots81 = glob.glob("/private/tmp/sigra-p2351-plan81-sequential-authority.*")
    require(not roots81, "Plan 81 root already exists before allocation")
    plan81_processes = [line for line in subprocess.check_output(["ps", "-axo", "command="], text=True).splitlines() if "plan81-" in line and "235.1-81-verify-preallocation.py" not in line]
    require(not plan81_processes, "Plan 81 process identity exists before allocation")
    for forbidden in (
        "235.1-81-LOCAL-TERMINAL.json",
        "235.1-PR-EVIDENCE.json",
        "235.1-SCAFFOLD-EVIDENCE.json",
        "235.1-VALIDATION-RUN.json",
    ):
        require(not (root / PHASE / forbidden).exists(), f"Plan 81 later-task effect already exists: {forbidden}")
    plan76 = validate_plan76_pair()
    plan77 = validate_plan77(inputs)
    plan78 = validate_plan78(inputs)
    plan79 = validate_plan79(inputs)
    plan80 = validate_plan80(inputs)
    ...
    new_identities = [row["action_id"] for row in schedule] + [row["raw_process_path"] for row in schedule] + [row["archive_path"] for row in schedule] + REMOTE_IDS + [ROOT_TEMPLATE, "plan81-local-terminal-launch", "plan81-install-golden-canonical-receiver", *DIAGNOSTIC.values()]
    for identity in map(str, new_identities):
        require(not any(prefix in identity for prefix in ("plan77-", "plan78-", "plan79-", "plan80-")), f"retired identity reused: {identity}")
    scheduler = scheduler_authority(root, inputs)
    contracts = run_contracts(root)
    plan_head = subprocess.check_output(["git", "log", "-1", "--format=%H", "--", str(PHASE / "235.1-81-PLAN.md")], cwd=root, text=True).strip()
    require(bool(re.fullmatch(r"[0-9a-f]{40}", plan_head)), "cannot bind Plan 81 introducing commit")
    return {
        "schema": SCHEMA,
        "status": "PASS",
        "repository": {"root": str(root), "authenticated_plan_head": plan_head},
        "precondition": {"status": "PASS", "plan81_root_count": 0, "plan81_process_record_count": 0},
        ...
        "partition_authority": {"producer": "fixed_sequential", "plan81_child_orchestration": "serial_one_shot_children", "fixed_receipt_paths": FIXED, "manifest_sha256": MANIFESTS},
        "root_template": ROOT_TEMPLATE,
        "local_terminal_launcher": "plan81-local-terminal-launch",
        "identity_schedule": schedule,
        "exact_six_action_id": "plan81-install-golden-canonical-receiver",
        ...
        "blacklists": {
            ...
            "plan79": {"root_namespace": "/private/tmp/sigra-p2351-plan79-status-recovery.*", "exact_root": str(PLAN79_ROOT), "id_prefix": "plan79-", "timing_action_ids": [f"plan79-repeat-{r}-partition-{p}" for r in (1, 2) for p in (1, 2)], "exact_six_action_id": "plan79-install-golden-canonical-receiver", "launcher": "plan79-local-terminal-launch", "remote_action_ids": [value.replace("plan81-", "plan79-") for value in REMOTE_IDS], "diagnostic_ids": ["plan79-classified-transient-diagnostic-retry", "plan79-classified-transient-diagnostic-run-attempt", "plan79-classified-transient-diagnostic-monitor"]},
            "plan80": {
                "root_namespace": "/private/tmp/sigra-p2351-plan80-sequential-authority.*",
                "id_prefix": "plan80-",
                "timing_action_ids": [f"plan80-repeat-{r}-partition-{p}" for r in (1, 2) for p in (1, 2)],
                "timing_process_paths": [f"local/process/plan80-repeat-{r}-partition-{p}.json" for r in (1, 2) for p in (1, 2)],
                "timing_archive_paths": [f"local/archive/plan80-repeat-{r}-partition-{p}.json" for r in (1, 2) for p in (1, 2)],
                "exact_six_action_id": "plan80-install-golden-canonical-receiver",
                "launcher": "plan80-local-terminal-launch",
                "remote_action_ids": [value.replace("plan81-", "plan80-") for value in REMOTE_IDS],
                "diagnostic_ids": ["plan80-classified-transient-diagnostic-retry", "plan80-classified-transient-diagnostic-run-attempt", "plan80-classified-transient-diagnostic-monitor"],
                "phase_local_paths": [
                    str(PHASE / "235.1-80-verify-preallocation.py"),
                    str(PHASE / "235.1-80-PREALLOCATION-VALIDATION.json"),
                    str(PHASE / "235.1-80-PREALLOCATION-FAILURE.json"),
                    str(PHASE / "235.1-80-SUMMARY.md"),
                ],
            },
        },
        "budgets": BUDGETS,
...
def main() -> int:
    ...
    parser.add_argument("--output", default=str(PHASE / "235.1-81-PREALLOCATION-VALIDATION.json"))
    ...
    require(
        json.loads(json.dumps(expected, sort_keys=True, separators=(",", ":"))) == expected,
        "independently derived authority is not JSON round-trip stable",
    )
    if receipt.exists():
    ...
    print("plan81-preallocation: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"plan81-preallocation: FAIL: {exc}", file=sys.stderr)
        raise SystemExit(1)
