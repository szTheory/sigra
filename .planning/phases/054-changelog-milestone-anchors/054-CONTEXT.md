...
- **D-13 (release cut):** On version bump **in the same commit** as the tag, **move the entire** `[Unreleased]` **block** into new `## [x.y.z] - date]`, then reset `[Unreleased]` to empty scaffolding. Prevents “upcoming” vs shipped contradiction.
