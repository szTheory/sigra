...
The D-11 MG-5/6 `test.fail()` is **not** about the static gallery board — it navigates to the *real* `/admin/audit` and user-audit pages (admin-design.spec.ts:342-385) and asserts pagination links render, which requires a user with ≥25 audit events (`@default_limit 25` in `lib/sigra/admin/audit/query_params.ex:22`). The example seeds file is an empty stub, and design-spec users are freshly registered per-test with ~3–5 events — so seeding ≥25 events for one user must happen in the example app's seeding path, not the static gallery markup.
...
**Why it happens:** `admin-design.spec.ts:342-385` does `page.goto('/admin/audit')` etc. and asserts `getByRole('link', { name: 'Previous page' })` is attached — pagination only renders at ≥25 rows. Admin audit `@default_limit 25` ([VERIFIED: lib/sigra/admin/audit/query_params.ex:22]). Fresh per-test users have ~3-5 events; the example `seeds.exs` is a 21-line empty stub ([VERIFIED]). The static gallery boards (`mg-5-desktop-results` etc.) render **hardcoded literal markup** with a static pagination nav (design_gallery_live.ex:611-833) — seeding does NOT touch them, so element-scoped baselines are unaffected.
...
1. **`board-notice` is the design-lane canary AND the font change alters every board including it.**
   ...
   - What's unclear: Whether the font-recapture should (a) treat this as a one-time canary re-establishment (delete + re-add so it counts as `added`), (b) temporarily relax the canary for this PR, or (c) pick a different design canary.
   - Recommendation: Treat the font-driven recapture as a **deliberate canary re-baseline** — this is the legitimate one-time event the canary's `added` tolerance is designed for IF the PNGs are net-new vs base; otherwise the planner should add an explicit, reviewed canary-override step scoped to this single recapture and reset afterward. Flag for the planner to decide; do not auto-relax silently.

2. **Recapture commit target: push-to-main vs open-a-PR.**
   - What we know: D-09 says "commits the PNGs through `snapshot-canary-guard.sh`"; the job runs on schedule/main/dispatch (non-PR).
   - What's unclear: branch-protection on `main` may reject a bot push; a recapture that auto-commits to main bypasses review of baseline changes.
   - Recommendation: Prefer **open-a-PR** (or push to a `ci/recapture-*` branch) so baseline changes get reviewed, unless the team explicitly wants auto-commit. Planner decides; both satisfy D-09's "recapture in CI" intent.

3. **Does any non-design lane depend on system-font metrics?** The font override changes `--font-sans` globally (`:root`). `admin-checkpoints` is fixed-viewport and tolerant (SEED-006 asymmetry), and demo-showcase has its own baselines.
   - Recommendation: After the font change, the **demo-showcase** and **admin-checkpoints** baselines may also shift (they now render Space Grotesk too). Verify whether those lanes' `maxDiffPixels` tolerance absorbs the change or whether they ALSO need recapture. This could widen the recapture scope beyond admin-design. **Planner must check** the demo-showcase and admin-checkpoints baselines after the font lands. Flag as a scope risk.
