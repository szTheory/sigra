...
1. **Assent 0.3.1 local-Google provider-double contract**
   - `Sigra.OAuth.Strategies.Google` forwards the generated provider keyword list unchanged apart from scope defaults. `Assent.Strategy.Google` merges its default `base_url` with that list, so generated-host `google: [base_url: local_base_url, ...]` overrides the Google issuer for both authorization and callback discovery. [VERIFIED: `lib/sigra/oauth/strategies/google.ex`, installed Assent 0.3.1 source]
   - The temporary host config must set `base_url: "http://127.0.0.1:<provider-port>"`, dummy `client_id`/`client_secret`, its generated callback as `redirect_uri`, `id_token_signed_response_alg: "HS256"`, and `code_verifier: true`. This keeps the real Google strategy while making the double deterministic; the host controller persists the PKCE verifier, whereas it does not persist an OIDC nonce, so the test must not configure one. [VERIFIED: `lib/sigra/oauth.ex`, generated OAuth controller, installed Assent 0.3.1 source]
   - The loopback double must expose `/.well-known/openid-configuration`, `/authorize`, and `/token`. Discovery returns absolute loopback `issuer`, `authorization_endpoint`, and `token_endpoint` values plus `token_endpoint_auth_methods_supported: ["client_secret_post"]`; `userinfo_endpoint` and `jwks_uri` are unnecessary for this exact HS256 Google-wrapper path. The authorization endpoint preserves the emitted signed `state`, checks the PKCE challenge is present, then redirects to the generated callback with a deterministic code. [VERIFIED: installed Assent 0.3.1 source]
   - `/token` accepts the authorization-code form body including the dummy client credentials and PKCE verifier, and returns an `access_token`, `token_type`, and a short-lived HS256 `id_token`. The ID token must contain `iss` equal to local `base_url`, `aud` equal to the dummy `client_id`, valid `iat`/`exp`, stable `sub`, and the collision user’s `email`, `email_verified`, `name`, and `picture` claims. Its HMAC key is the dummy `client_secret`. This is sufficient because the Google wrapper uses `OIDC.fetch_user/2`, which validates the ID token and does not invoke `fetch_userinfo/2`. [VERIFIED: installed Assent 0.3.1 source]
   - Wave 0 remains a retained runtime proof of this pinned contract, not an unresolved design spike: it must assert that the browser begins at generated `/auth/google`, reaches only the loopback authorization endpoint, returns the exact state through the real callback, exchanges the code with PKCE, and lands on the generated existing-email collision outcome.
