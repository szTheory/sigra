...
- **D-06 [deferred]:** Phase 156 LiveViews will consume the module via `import Sigra.Admin.Components` (not `use`) and delete the now-duplicated private `defp` defs. (Out of scope for 155 — a forward-reference for Phase 156, NOT a Phase 155 deliverable; recorded so the planner builds toward it. Plans intentionally do not cover this; the keystone leaves the module unwired.)
