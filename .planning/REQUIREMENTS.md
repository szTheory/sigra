...
- [x] **PK-UX-01**: User can enroll a passkey from account settings. Enrollment is gated by `Sigra.Plug.RequireSudo` — user must re-authenticate with password or TOTP before enrollment (addresses pitfall P-2 — stolen-session takeover).
...
- [x] **PK-UX-05**: User can log in via passkey as a second factor alongside TOTP on the MFA prompt (v1.1 default MFA mode for users with both). Backup codes remain available.
...
- [x] **PK-UX-10**: Passkey enrollment and authentication LiveViews use Phoenix JS hooks (`PasskeyRegister`, `PasskeyAuthenticate`) wrapping `@simplewebauthn/browser` — no vanilla base64url plumbing.
...
- [x] **PK-UX-12**: Passkey JS hooks handle browser abort, timeout, user-cancel, and AbortController scenarios cleanly — LiveView returns to a recoverable state with a clear error message (addresses pitfall P-8).
