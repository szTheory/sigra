...
| `npx playwright show-trace` | Post-mortem of a CI flake | **Corrected post-236-01/236-03:** `playwright.config.ts:59` hardcodes `retries: 0`, so `trace: 'on-first-retry'` (`:81`) never fires and no trace artifact for a flaky run has ever existed; the `PLAYWRIGHT_RETRIES: 1` env key (formerly `ci.yml:1460`) had zero readers and has since been deleted (236-03). No trace can be harvested — a RED must be manufactured deliberately, as 236-01 did (see `236-EVIDENCE.md`'s `BEFORE-FLAKE-RED` slot: a real CI run plus a local `--repeat-each --trace=on` reproduction under genuine host CPU contention) |
...
1. **Corrected post-236-01/236-03: no trace exists and none can be harvested.** `playwright.config.ts:59` hardcodes `retries: 0`, so `trace: 'on-first-retry'` never fires on a first-attempt failure, and the `PLAYWRIGHT_RETRIES: 1` env key had zero readers (and has since been deleted, 236-03). A RED must be manufactured deliberately — 236-01 did this with a real, unprompted CI failure (run `35004420339`) corroborated by a local `--repeat-each --trace=on` reproduction under genuine host CPU contention (see `236-EVIDENCE.md`'s `BEFORE-FLAKE-RED` slot).
