...
## Current Milestone: v1.39 DS-COHERENCE

**Goal:** Turn the strengthened v1.38 brand into a systematically audited, award-grade admin/operator design system — evaluated *fractally* at every level (individual components → repeatable component groups → page compositions → operator flows) and shipped to real adopters by closing the admin-CSS distribution gap, governed by a re-runnable quality ledger so re-runs only ever move quality forward.

**Lane (MILESTONE-ARC):** Strategic bet — user-promoted exception to "polish is not default roadmap" (same justification class as v1.34/v1.38: the admin/operator UI is the evaluator-facing + generated-host adoption surface). Also closes a real production-readiness gap: the admin UI ships effectively *unstyled* to generated hosts today (the `sg-*` CSS is example-trapped).

**Target features:**
- Close the distribution gap: extract the canonical admin `sg-*` CSS into a shipped installer asset, with merge-blocking example≡template parity and a styled generated-host proof
- Hand-rolled, example-only `/admin/_design` gallery + an `admin-design-{chromium,mobile,dark}` snapshot lane, so every component and component-group is auditable in isolation across all states/themes/widths
- Idempotency machinery: a quality-tier ledger (Drift / Ratified / Award-grade) + a merge-blocking monotonic guard + a ratified fractal scorecard rubric (D1–D11 + level add-ons)
- Fractal quality audit + improvement: tokens (L0) → individual components (L1) → meta-component groups (L2) → page compositions (L3) → operator flows (L4), with emilkowal.ski-aligned motion, GOV.UK information architecture, correct Light/Dark/System, mobile-first responsive, a11y (WCAG AA), and on-brand UX microcopy
- Deterministic seed/fixture enrichment so happy/error/boundary flows are exercised; a system-wide microcopy + IA sweep with a one-term-per-concept glossary

**Methodology:** every decision point is researched with subagents (best practices, anti-patterns, footguns, real user loved/hated feedback), adversarially judged, then synthesized — per GSD research→plan→execute→verify.

**Out of scope:** generated auth-UI design audit (token coherence preserved only); re-litigating v1.34 "same job → same component" winners; brand/logo rework (done v1.38); Tailwind as admin source of truth; per-org branding; new admin features/screens/nav; README/social adoption (gsd-quick fast-follow); a `phx_storybook` dependency.

**Approved kickoff plan:** `~/.claude/plans/design-system-stress-test-serialized-candy.md`
