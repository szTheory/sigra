...
## Current Milestone: v1.44 ADMIN-UX-RATCHET

**Goal:** Automate the evaluation and forward-only ratcheting of admin/operator UI/UX quality — adversarial LLM panels (per user-persona/JTBD + a graphic-design/visual-quality lens) plus deterministic visual probes — so award-grade iteration is fast and only-moves-forward, needing a human only for a minimal final sign-off rather than an endless manual "hunt for issues → fix a few → repeat" loop. Then apply the harness in one broad wave to elevate every admin surface.

**Thesis:** After nine consecutive manual admin-UI/brand/DS milestones (v1.34–v1.42) drove all 36 fractal quality-ledger cells to Tier-2, the bottleneck is no longer the design — it's the *tedious hand-labor* of evaluating and defending that quality. This milestone builds the automation that replaces the hunt: a deterministic-first, LLM-residual, forward-only harness. The durable merge-blocking signal stays 100% deterministic (monotonic ledger guard, snapshot-canary, axe, token/contrast gates); the LLM is only an issue-finder + judge-of-taste, never a CI gate and never flips a ledger cell. Two load-bearing lessons are designed around: **cite-and-flip** (evaluate RENDERED output, never code) and **LLM-nondeterminism** (keep the judge out of the merge-blocking path).

**Target features:**
- **Evaluation harness** — a near-single-command loop that renders every admin surface (light/dark/mobile × populated/zero/loading/error) into evidence bundles, runs deterministic visual probes (promoting the 3 documented-as-manual scorecard proxies: motion, whitespace rhythm, target-size) + an adversarial 4-lens LLM panel (3 persona/JTBD + 1 graphic-design), and emits a prioritized, deduplicated, forward-only fix queue.
- **Forward gradient (verify-then-climb)** — re-verify existing Tier-2 claims against rendered output (catching optimistically cite-and-flipped cells), then a finer-grained award sub-score above the current ceiling to keep climbing.
- **Auto-fix with safety rails** — auto-apply provably-safe mechanical fixes (copy/token/component-swap) with per-fix auto-revert on regression; human reviews the batched PR + signs off judgment calls and gradient raises.
- **Broad elevation wave** — apply the harness across all 8 admin surfaces + the L1/L2 component fractal, folding in UI-01 (demo-DX nits) + UI-02 (Tasklane rebrand residuals), then recapture baselines + terminal ratification.

Phases 216–220 (lean: automation + one strong wave). **Deferred / out of scope:** FEAT-01/02/03 (config/installer features — a later feature milestone), non-admin/auth surfaces, SEED-005 (CI-perf). Full design: `~/.claude/plans/a-pure-ui-ux-cleanup-peaceful-star.md`.
