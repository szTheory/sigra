# Sigra Hex 1.32.0 Launch Evidence Bundle

This bundle is the compact attachable evidence page for the Sigra Hex 1.32.0 launch. It routes to canonical proof and states proof boundaries; it does not duplicate the full release runbook, UAT matrix, or demo walkthrough.
...
| --- | --- | --- | --- |
| Release gate matrix | [Release runbook v1.32](release-runbook-v1-0.html#release-gate-matrix) (`docs/release-runbook-v1-0.md`) | Gate list, ref rules, required evidence, and waiver fields are defined | Fill release-ref run URLs and waiver rows, if any |
...
| Hex visibility | [Release runbook post-publish visibility](release-runbook-v1-0.html#post-publish-visibility) | Placeholder only | Replace `POST_PUBLISH_HEX_VISIBILITY_URL` after Hex shows `1.32.0` |
...
For release proof hosted outside the Hex package tarball, use pinned `v1.32.0` links instead of `main` blob URLs. In other words: do not use main blob URLs for release evidence. This applies to GitHub-hosted planning artifacts, run logs, screenshots, and any release evidence that would otherwise drift after the release.
