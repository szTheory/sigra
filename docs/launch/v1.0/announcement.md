# Sigra Hex 1.32.0 Announcement

Sigra Hex 1.32.0 is the stability and trust release for Phoenix 1.8+ authentication teams that want normal Phoenix code in their application and an updateable security core from Hex.
...
## What changed at Hex 1.32.0

The public 1.32 line locks Sigra's package contract around Phoenix 1.8+, generated-host ownership, security invariants, SemVer/deprecation policy, upgrade guidance, migration lanes, demo proof, release gates, and evidence routing.
...
Sigra Hex 1.32.0 does not claim to provide:
...
- SCIM support in the 1.32 release.
...
- [Release runbook v1.32](release-runbook-v1-0.html) - maintainer release gate matrix, publish checks, recovery path, and first-14-day hotfix policy. Repo path: `docs/release-runbook-v1-0.md`.
...
- [Sigra 1.32 contract](../../../guides/introduction/contract.md) - installable package truth, ownership boundaries, SemVer policy, security invariants, and non-goals. Repo path: `guides/introduction/contract.md`.
...
- You want the selected Hex `1.32.0` contract line for Phoenix 1.8+ auth.
...
The release runbook defines the first-14-day hotfix policy for Hex `1.32.0`: P0/P1 issues are same-business-day triage, P2 issues are reviewed within 24 hours, and P3 polish is batched into planned patch work. Every intake needs a repro command/ref, failing run URL or log, affected version, and workaround status.

See the [Release runbook v1.32](release-runbook-v1-0.html#first-14-days-hotfix-policy) for the canonical severity and patch decision boundaries.
