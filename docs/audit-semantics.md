...
**C-1 matrix (post–v1.4 inventories):** the authoritative “which boundary is T1 vs intentional T2 (`EX-*`)” table lives in [`.planning/phases/09-audit-logging/09-VERIFICATION.md`](../.planning/phases/09-audit-logging/09-VERIFICATION.md) — keep that file aligned with `43-` / `44-` / `45-` **AUD-04** inventories when audit mechanics change.
