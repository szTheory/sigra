...
<%= if adapter == :postgres do %>
  ...
  def up do
    # ── Organizations ──────────────────────────────────────────────────
    create table(:organizations, Keyword.merge(@prefix_opts, <%= if binary_id, do: "[primary_key: false]", else: "[]" %>)) do
...
<% end %>      add :name, :string, null: false, size: 255
    ...
    create table(:organization_memberships, Keyword.merge(@prefix_opts, <%= if binary_id, do: "[primary_key: false]", else: "[]" %>)) do
<%= if binary_id do %>      add :id, :binary_id, primary_key: true
<% end %>      add :role, :string, null: false, default: "member"
      add :organization_id, references(:organizations, Keyword.merge(@ref_opts, <%= if binary_id, do: "[type: :binary_id, on_delete: :delete_all]", else: "[on_delete: :delete_all]" %>)), null: false
      add :user_id, references(:<%= table_name %>, Keyword.merge(@ref_opts, <%= if binary_id, do: "[type: :binary_id, on_delete: :delete_all]", else: "[on_delete: :delete_all]" %>)), null: false
    ...
    create table(:organization_invitations, Keyword.merge(@prefix_opts, <%= if binary_id, do: "[primary_key: false]", else: "[]" %>)) do
<%= if binary_id do %>      add :id, :binary_id, primary_key: true
<% end %>      add :email, :citext, null: false
      ...
      add :organization_id, references(:organizations, Keyword.merge(@ref_opts, <%= if binary_id, do: "[type: :binary_id, on_delete: :delete_all]", else: "[on_delete: :delete_all]" %>)), null: false
      add :invited_by_id, references(:<%= table_name %>, Keyword.merge(@ref_opts, <%= if binary_id, do: "[type: :binary_id, on_delete: :nilify_all]", else: "[on_delete: :nilify_all]" %>))
      add :accepted_by_id, references(:<%= table_name %>, Keyword.merge(@ref_opts, <%= if binary_id, do: "[type: :binary_id, on_delete: :nilify_all]", else: "[on_delete: :nilify_all]" %>))
      add :revoked_by_id, references(:<%= table_name %>, Keyword.merge(@ref_opts, <%= if binary_id, do: "[type: :binary_id, on_delete: :nilify_all]", else: "[on_delete: :nilify_all]" %>))
...
<%= if binary_id do %>      add :id, :binary_id, primary_key: true
<% end %>      add :organization_id, references(:organizations, Keyword.merge(@ref_opts, <%= if binary_id, do: "[type: :binary_id, on_delete: :delete_all]", else: "[on_delete: :delete_all]" %>)), null: false
