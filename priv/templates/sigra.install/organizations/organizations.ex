defmodule <%= app_module %>.Organizations do
  ...
  alias <%= context_module %>.OrganizationAuthPolicy
  alias <%= context_module %>.OrganizationAuthPolicyExemption
  alias <%= repo_module %>
  ...
  def list_auth_policy_exemptions(scope) do
    ...
    |> join(:inner, [row], membership in <%= context_module %>.OrganizationMembership,
      on:
        membership.organization_id == row.organization_id and
          membership.user_id == row.user_id
    )
    |> join(:inner, [_row, membership], user in <%= context_module %>.<%= schema_alias %>,
  ...
  def list_break_glass_candidates(scope) do
    <%= context_module %>.OrganizationMembership
    |> where([membership], membership.organization_id == ^scope.active_organization.id)
    |> join(:inner, [membership], user in <%= context_module %>.<%= schema_alias %>,
      on: user.id == membership.user_id
    )
