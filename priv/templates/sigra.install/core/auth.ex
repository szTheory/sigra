...
<% end %>
  ...
  def resend_user_confirmation_instructions(%<%= schema_alias %>{} = user, confirmation_url_fun)
      when is_function(confirmation_url_fun, 1) do
...
<%= if organizations?, do: "    auth_policy = user && #{app_module}.Organizations.local_auth_policy_for(user)", else: "    auth_policy = nil" %>
    ...
    case Sigra.Auth.request_password_reset(Repo, normalized_email,
           ...
           window_ms: runtime_positive_integer(:reset_rate_limit_window, 60_000)<%= if organizations?, do: ",\n           enterprise_auth_policy: #{app_module}.Organizations", else: "" %>
         ) do
      {:ok, {signed_token, url}} ->
        if user do
...
<%= if passkeys? do %>
  ...
  def authenticate_discoverable_passkey(assertion_params) do
    with {:ok, normalized_params} <- normalize_passkey_assertion_params(assertion_params, Map.get(assertion_params, "challenge") || Map.get(assertion_params, :challenge)),
         ...
         %<%= schema_alias %>{} = user <- Repo.get(<%= schema_alias %>, passkey.user_id),
